...
def notify_users_on_new_course(doc, method):
    # Get all users who have the "LMS Student" role and are enabled
    ...
    # Send email to all LMS Students
    if emails:
        frappe.sendmail(
            recipients=emails,
            subject=f"New Course Added: {doc.title}",
            message=f"A new course '{doc.title}' has been added to the platform.",
            now=True
        )
